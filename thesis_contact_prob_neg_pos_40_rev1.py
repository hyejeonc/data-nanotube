...
for j in range(len(pathstring)):
    ...
    for p in sorted(Path(str(pathstring[j])).glob('zero/ph*/cylinder_shell.list')):
        ...
        '''    
        axs[0].fill_between(d[:,0], d_errdown, d_errup, color = '{:s}'.format(linecolor[int(p.parts[-2][2])-2]) , alpha=0.2)
        ...
        axs[1].fill_between(e[:,0], e_errdown, e_errup, color = '{:s}'.format(linecolor[int(p.parts[-2][2])-2]) , alpha=0.2)

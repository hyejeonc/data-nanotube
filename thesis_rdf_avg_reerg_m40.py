...
'''
['m40ncenxyz',  'm40pcenxyz', 'm40n', 'm40p',\
...
for j in range(len(pathstring)):
    ...
    for p in sorted(Path(str(pathstring[j])).glob('zero/ph*/cylinder_shell.out')):
        ...
        reeavg[j].append(r_single[0][4][0]) #reeavg 
        rgavg[j].append(r_single[0][5][0])  #rgavg 
        ...
        if startcount == 1:
            #reesum = ree
            ree.to_csv("thesis_avg_rev1.csv", mode="w")
            #rg.to_csv("spssdata/inout_spss_rev0_sym.csv", mode="a")
        else:    
            ree.to_csv("thesis_avg_rev1.csv", mode="a", header=False)
...
#df = pd.read_csv('thesis_avg_rev1.csv') # df is pandas.DataFrame
#ax1 = df.plot.bar(x='ph', y='reeavg')

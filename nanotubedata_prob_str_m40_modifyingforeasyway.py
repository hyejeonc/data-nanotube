...
for i in strtype:
    for p in sorted(Path('alsi/polymer/quenchedpoly/{:s}'.glob('/zero/ph*/cylinder_shell.list')):
        print(p)  
...
for p in sorted(Path('alsi/polymer/quenchedpoly/m20m20/zero/').glob('ph*/cylinder_shell.list')):
    print(p)  
...
plt.ylabel('Prob(d<7.1Å)')
plt.xlabel('Monomer index')
plt.title('m10m10m10m10'+'\n Probability of inner/outer tube surface')
plt.show()

for p in sorted(Path('alsi/polymer/quenchedpoly/m10m10m10m10/zero/').glob('ph*/cylinder_shell.list')):
    print(p)  
    d = mu.monAnalysis(p, 'otube', 'tt')    
#    labelstring = 'ph = {:d}'.format( int(p.parts[1][2]) )
    labelstring = 'ph = {:d}'.format(int(p.parts[-2][2])) 
    print(p.parts)
    print(p.parts[-2])

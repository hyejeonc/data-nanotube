...
strtype = ['m40ncenxyz',  'm40pcenxyz', 'm40nretry', 'm40pretry'] #,\
...
#        print(outseg)
        ...
        e = {str(p.parts[-4]) : r_single[0][4][0]  }
        ree = pd.DataFrame(data = d) 
        ree['reeerr'] = np.sqrt((r_single[0][4][1])**2 - (r_single[0][4][0])**2)
        
        ree['rgrms'] = r_single[0][5][1]
        ree['rgavg'] = r_single[0][5][0]
        
        ree['rgerr'] = np.sqrt((r_single[0][5][1])**2 - (r_single[0][5][0])**2)
        ...
        if startcount == 1:
            #reesum = ree
            ree.to_csv("inout_table_rev0.csv", mode="w")
            
            #rg.to_csv("inout_table_rev0.csv", mode="a")
        else:    
            ree.to_csv("inout_table_rev0.csv", mode="a", header=False)
            #rg.to_csv("inout_table_rev0_sym.csv", mode="a", header=False)
...
#gr1 = df.groupby('structure', as_index=False)


#gr1.plot.bar('ph', 'reeavg')
#exp_distribution.unstack(['Gender','Interest'])['Male'].plot(kind='bar') 
#gr1.unstack(['structure']).plot.bar('ph', 'reeavg') 
#plt.show()
#x = plt.bar()
#x.append(gr1.plot.bar('ph', 'reeavg'))
#df.plot.legend()